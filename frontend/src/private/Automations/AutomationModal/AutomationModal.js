    ...
    getIndexes(token)
      .then((indexes) => {
        const filteredIndexes = indexes.filter(
          (k) => k.symbol === automation.symbol
        );
        const baseWallet = indexes.find(
          (ix) =>
            ix.variable === "WALLET" && automation.symbol.startsWith(ix.symbol)
        );
        if (baseWallet) filteredIndexes.splice(0, 0, baseWallet);

        const quoteWallet = indexes.find(
          (ix) =>
            ix.variable === "WALLET" && automation.symbol.endsWith(ix.symbol)
        );
        ...
        <div className="modal-content">
          <div className="modal-header">
            <p className="modal-title" id="modalTitleNotify">
              {props.data.id ? "Edit " : "New "}Automation
            </p>
            <button
              ref={btnClose}
              type="button"
