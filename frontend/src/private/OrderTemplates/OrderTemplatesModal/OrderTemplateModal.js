...
function OrderTemplateModal(props) {
  ...
  useEffect(() => {
    ...
    getIndexes(token)
      .then((indexes) => {
        const indexesRegex =
          /^(BOOK|LAST_CANDLE|LAST_ORDER.(limitPrice|stopPrice|avgPrice))/;
        const filteredIndexes = indexes.filter(
          (k) =>
            k.symbol === orderTemplate.symbol && indexesRegex.test(k.variable)
        );
        ...
        <div className="modal-content">
          <div className="modal-header">
            <p className="modal-title" id="modalTitleNotify">
              {orderTemplate.id ? "Edit" : "New"} Order Template
            </p>
            <button
              ref={btnClose}
              type="button"
                ...
                <div className="col-12 mb-3">
                  <div className="form-group">
                    ...
                    <input
                      ...
                      defaultValue={orderTemplate.name}
                      onChange={onInputChange}
                    />
                  </div>
                </div>
              ...
              <div className="row">
                ...
                <div className={getIcebergClasses(orderTemplate.type)}>
                  <QuantityTemplate
                    id="icebergQty"
                    text="Iceberg Qty:"
                    quantity={orderTemplate.icebergQty}
                    multiplier={orderTemplate.icebergQtyMultiplier}
                    onChange={onInputChange}
                  />
                </div>
              </div>
