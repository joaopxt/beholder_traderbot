...
function getAnalysisIndexes() {
  return {
    ...
    [indexKeys.KST]: {
      params:
        "roc1, roc2, roc3, roc4, smaroc1, smaroc2, smaroc3, smaroc4, signal",
      name: "KST",
    },
    ...
    [indexKeys.INSIDE_BAR]: { params: "bars", name: "Inside Bar" },
    ...
    [indexKeys.BULLISH_MARUBOZU]: { params: "none", name: "Bullish Maruzobu" },
    [indexKeys.BEARISH_MARUBOZU]: { params: "none", name: "Bearish Maruzobu" },
...
function execCalc(indexName, ohlc, ...params) {
  switch (indexName) {
    ...
    case indexKeys.BEARISH_ENGULFING:
      return bearishEngulfing(ohlc, ...params);
    ...
    case indexKeys.INSIDE_BAR:
      return insideBar(ohlc, ...params);
    case indexKeys.KST:
...
function getInsideBar(ohlc, bars, last) {
  let hasInsideCandle =
...
function insideBar(ohlc, bars = 1) {
  const last = ohlc.high.length - 1;
  const current = getInsideBar(ohlc, bars, last);
  const previous = getInsideBar(ohlc, bars, last - 1);
...
//FUNÇÕES DE CANDLES PARA ANÁLISE GRÁFICA
